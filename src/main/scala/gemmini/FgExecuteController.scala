...
class FgExecuteController[T <: Data](config: FgGemminiArrayConfig[T])
  ...
  io.readB.req.bits.row := b_read_counter
  io.readB.req.bits.fg_col_start := b_fg_col_start
  io.readB.req.bits.bank_start := b_bank_start
  io.readB.req.bits.banks := b_total_banks

  val b_read_data = io.readB.bits.resp.data
  ...
  io.readA.req.bits.row := a_read_counter
  io.readA.req.bits.fg_col_start := a_fg_col_start
  io.readA.req.bits.bank_start := a_bank_start
  io.readA.req.bits.banks := a_total_banks

  val a_read_data = io.readA.bits.resp.data
 ...
 //   (!cntlq.d_fire || mesh.io.d.fire() || !mesh.io.d.ready)
  ...
  io.writeC.en := is_array_outputting_valid_row
  io.writeC.row := output_row
  io.writeC.cols := w_matrix_cols
  io.writeC.fg_col_start := w_fg_col_start
  io.writeC.bank_start := w_bank_start
  io.writeC.banks := w_banks
  io.writeC.accum := w_accum
