...
//============================================================================
class DMASplitter[T <: Data](config: GemminiArrayConfig[T])
  ...
  val io = IO(new Bundle {
    ...
    val txn = Flipped(Decoupled(new DMADispatch(config)))
    val peek = new DMATrackerPeekIO(config)
  ...
  req.ready := false.B
  val cur_req = Reg(new DMAStoreRequest(config))
  
  // current txn that we are splitting into beats
  val txn = Queue(io.txn, 4)
  txn.ready := false.B
  val cur_txn = Reg(new DMADispatch(config))
  ...
  val beat_data_rshift   = Wire(UInt(log2Ceil(ACC_ROW_BITS+DMA_TXN_BITS).W))
