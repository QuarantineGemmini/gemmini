...
//===========================================================================
// request to DMAControl to start dma write/read operation
//===========================================================================
class DMAControlRequest[T <: Data](val config: GemminiArrayConfig[T])
...
//===========================================================================
// request from DMAControl to the circuit that sends tilelink-A reqs
//===========================================================================
class DMADispatch[T <: Data](val config: GemminiArrayConfig[T])
  (implicit p: Parameters) extends CoreBundle {
  import config._
...
//===========================================================================
// DMAControl is same controller for DMA reads/writes
// - it handles translating vaddrs to paddrs
// - it handles splitting operations into tilelink transactions
// - it allocates entries in the DMATracker for each tilelink txn initiated
// - achieves max throughput of 1 dispatch/cycle
//===========================================================================
class DMAControl[T <: Data]
  (config: GemminiArrayConfig[T], is_load_mode:Boolean)
  ...
  val io = IO(new Bundle {
    val req        = Flipped(Decoupled(new DMAControlRequest(config)))
    val txn_nextid = Input(UInt(DMA_REQS_IDX.W))
    val alloc      = Decoupled(new DMATrackerEntry(config))
    val req_curid  = Input(UInt(DMA_REQS_IDX.W))
    val incr       = Valid(new DMAReqIncr(config))
    val dispatch   = Decoupled(new DMADispatch(config))
  ...
  // active request
  //-----------------------------------------------
  val req = Reg(new DMAControlRequest(config))
  ...
  val total_useful_bytes     = RegInit(0.U(ACC_ROW_BYTES_CTR.W))
  val useful_bytes_left      = RegInit(0.U(ACC_ROW_BYTES_CTR.W))
  val total_bytes_requested  = RegInit(0.U(ACC_ROW_BYTES_CTR.W))
