...
class XactTrackerAllocIO
  (val nXacts: Int, val maxShift: Int, val spadWidth: Int, val accWidth :Int,
...
//  maxShift:  the maximum number of bytes in the beginning of a 
//             TileLink response which may be discarded
...
class FgXactTracker[T <: Data](
  ...
  val free_entry = MuxCase((nXacts-1).U, entries.zipWithIndex.map { 
