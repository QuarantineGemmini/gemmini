...
class Pipeline[T <: Data]
  (gen: T, latency: Int)
  (comb: Seq[T => T] = Seq.fill(latency+1)((x: T) => x)) 
  extends Module 
{
  ...
  if (latency == 0) {
    ...
    // Valid signals
    // When the pipeline stage ahead of you isn't stalling, 
    // then make yourself invalid
    ...
    ((stages.tail zip stages.init) zip 
        (stalling.tail zip comb.tail.init)).foreach { 
      case ((st2, st1), (s2, c1)) =>
      when(!s2) {
        st2 := c1(st1)
      }
...
object Pipeline {
  def apply[T <: Data](in: ReadyValidIO[T], latency: Int, 
                       comb: Seq[T => T]): DecoupledIO[T] = {
